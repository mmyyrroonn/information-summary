...
function buildClassificationPrompt(batch: SampleTweet[]) {
  ...
  const template = {
    ...
    constraints: [
      ...
      '地缘政治/军事事件评估：涉及主要经济体或产油国（美国/中国/俄罗斯/中东）的军事冲突、制裁、战争进展，如果可能影响能源/大宗商品/全球供应链/市场情绪，至少 importance=3 且 watch。知名分析师（如Niall Ferguson等）对市场影响的深度分析也至少 watch。',
...
async function main() {
  ...
  const samplePath = path.resolve(dataDir, `sample-${type}.json`);
