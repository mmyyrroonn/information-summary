...
const envSchema = z.object({
  ...
  SOCIAL_DIGEST_MINIMAX_MODEL: z.string().default('MiniMax-M2.5'),
  ...
  LEGACY_FETCH_ENABLED: z.coerce.boolean().default(false),
  LEGACY_CLASSIFY_ENABLED: z.coerce.boolean().default(false),
  FETCH_CRON_SCHEDULE: z.string().default('0 */2 * * *'),
  CLASSIFY_CRON_SCHEDULE: z.string().default('*/5 * * * *'),
  FETCH_BATCH_SIZE: z.coerce.number().default(10),
  FETCH_COOLDOWN_HOURS: z.coerce.number().default(2),
  SOURCE_LIST_FETCH_CRON_SCHEDULE: z.string().default('0 */2 * * *'),
  SOURCE_LIST_FETCH_BATCH_SIZE: z.coerce.number().default(20),
  SOURCE_LIST_SOURCE_COOLDOWN_HOURS: z.coerce.number().default(2),
  ...
  CLASSIFY_TRUSTED_SOURCE_MODE: z.coerce.boolean().default(false),
